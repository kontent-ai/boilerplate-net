...
namespace Kentico.Kontent.Boilerplate.Services
{
    public class ReactiveCacheManager : ICacheManager
    {
        ...
        public void CreateEntry<T>(IEnumerable<string> identifierTokens, T value, Func<T, IEnumerable<IdentifierSet>> dependencyListFactory)
        {
            ...
            {
                var dummyIdentifierTokens = new List<string> { CacheHelper.DUMMY_IDENTIFIER, dependency.Type, dependency.Codename };
        ...
        public void InvalidateEntry(IdentifierSet identifiers)
        {
            ...
            {
                if (MemoryCache.TryGetValue(StringHelpers.Join(CacheHelper.DUMMY_IDENTIFIER, typeIdentifier, identifiers.Codename), out CancellationTokenSource dummyEntry))
                {
